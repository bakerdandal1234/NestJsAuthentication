...
describe('StripeWebhookService payment confirmation', () => {
  ...
  const request = (
    ...
    const body = JSON.stringify({ id: eventId, type, data: { object: value } });
    return {
  ...
  beforeEach(() => {
    ...
    intent = {
      id: 'pi_test',
      metadata: { flowdeskPaymentId: paymentId, orderId },
      status: 'succeeded',
      currency: 'usd',
      amount: 10000,
    ...
    const insert: any = {};
    for (const name of ['insert', 'values', 'orIgnore'])
      insert[name] = jest.fn().mockReturnValue(insert);
    ...
    refundsService = { synchronize: jest.fn().mockResolvedValue({}) };
    service = new StripeWebhookService(
      ...
      dataSource,
      refundsService as RefundsService,
    );
  });

  it('confirms the pending order and saves payment success before marking the event processed', async () => {
    await expect(service.handleWebhook(request(intent))).resolves.toMatchObject(
      { received: true },
    );
    expect(payment.status).toBe(PaymentStatus.SUCCESS);
    expect(payment.paidAt).toBeInstanceOf(Date);
    expect(order.status).toBe(OrderStatus.CONFIRMED);
    expect(orders.save.mock.invocationCallOrder[0]).toBeLessThan(
      events.update.mock.invocationCallOrder[0],
    );
  ...
  it.each([OrderStatus.CONFIRMED, OrderStatus.COMPLETED, OrderStatus.REFUNDED])(
    'does not regress a %s order when another event reports the same successful payment',
    async (status) => {
  ...
  it.each(['amount', 'amount_received', 'currency', 'id', 'orderId'])(
    'rejects mismatched %s without acknowledging it as processed',
    async (field) => {
      if (field === 'orderId') intent.metadata.orderId = 'other-order';
      else
        intent[field] =
          field === 'amount' || field === 'amount_received' ? 1 : 'wrong';
      await expect(
        service.handleWebhook(request(intent)),
  ...
  it('rejects an invalid signature before opening a transaction', async () => {
    ...
    await expect(service.handleWebhook(req)).rejects.toThrow(
  ...
  it.each(['refund.created', 'refund.updated', 'refund.failed'])(
    'synchronizes %s before marking the signed event processed',
    async (type) => {
      const value = {
        id: 're_test',
        metadata: { flowdeskRefundId: paymentId },
        status: 'pending',
      };
      await service.handleWebhook(request(value, 'evt_refund', type));
      expect(refundsService.synchronize).toHaveBeenCalledWith(
        expect.any(Object),
        're_test',
        paymentId,
      );
      expect(
        refundsService.synchronize.mock.invocationCallOrder[0],
      ).toBeLessThan(events.update.mock.invocationCallOrder[0]);
      expect(payments.save).not.toHaveBeenCalled();
    },
  );

  it('does not process the same refund event twice', async () => {
    const req = request({ id: 're_test' }, 'evt_refund', 'refund.updated');
    await service.handleWebhook(req);
    await service.handleWebhook(req);
    expect(refundsService.synchronize).toHaveBeenCalledTimes(1);
  });

  it('leaves a failed refund synchronization unprocessed for redelivery', async () => {
    refundsService.synchronize.mockRejectedValue(
      new Error('Stripe unavailable'),
    );
    await expect(
      service.handleWebhook(
        request({ id: 're_test' }, 'evt_refund', 'refund.failed'),
      ),
